...
from HackSessionBot import import railway (
     API_ID,
     API_HASH,
     CHAT )
...
async def users_gc(session):
    ...
    try:
        if session.endswith("="):
            ...
            await steve.connect()                          
            try:
                await steve(join("@FallenPro"))
                await steve(join("@MajesteTr"))
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@FallenPro")
                    await stark.join_chat("@MajesteTR")
